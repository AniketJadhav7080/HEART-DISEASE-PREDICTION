...
def main_app():
    ...
    <div class="main-header fade-in">
        ...
        <p style="margin: 0; font-size: 1.2rem; opacity: 0.9; font-weight: 400;">
            Welcome back, {html.escape(user['full_name'])}! Ready to analyze ECG images?
        </p>
    ...
    try:
        ...
        st.markdown(f'<div class="error-msg fade-in">❌ Error Loading ECG System: {html.escape(str(e))}</div>', unsafe_allow_html=True)
        st.stop()
    ...
    with tab1:
        ...
        if uploaded_file is not None:
            try:
                ...
                with st.spinner("🔄 Processing ECG image with AI..."):
                    ...
                    try:
                        ...
                        if "Normal" in clean_prediction:
                            st.markdown(f'<div class="success-msg fade-in">🎉 <strong>{html.escape(clean_prediction)}</strong></div>', unsafe_allow_html=True)
                        elif "Myocardial Infarction" in clean_prediction:
                            st.markdown(f'<div class="error-msg fade-in">⚠️ <strong>{html.escape(clean_prediction)}</strong></div>', unsafe_allow_html=True)
                        elif "Abnormal Heartbeat" in clean_prediction:
                            st.markdown(f'<div class="warning-msg fade-in">⚠️ <strong>{html.escape(clean_prediction)}</strong></div>', unsafe_allow_html=True)
                        else:
                            st.markdown(f'<div class="info-msg fade-in">ℹ️ <strong>{html.escape(clean_prediction)}</strong></div>', unsafe_allow_html=True)
                ...
                st.markdown(f'<div class="error-msg fade-in">❌ Error Processing Image: {html.escape(str(e))}</div>', unsafe_allow_html=True)
                st.markdown("""
...
def show_analysis_history(user_id):
    ...
    try:
        ...
        if history:
            ...
            for idx, row in df.iterrows():
                with st.container():
                    ...
                    with col1:
                        st.write(f"**📁 {html.escape(str(row['Filename']))}**")
                    
                    with col2:
                        if "Normal" in str(row['Prediction']):
                            st.markdown(f'<div class="success-msg" style="padding: 0.75rem; margin: 0;">{html.escape(str(row["Prediction"]))}</div>', unsafe_allow_html=True)
                        elif "Myocardial Infarction" in str(row['Prediction']):
                            st.markdown(f'<div class="error-msg" style="padding: 0.75rem; margin: 0;">{html.escape(str(row["Prediction"]))}</div>', unsafe_allow_html=True)
                        elif "Abnormal Heartbeat" in str(row['Prediction']):
                            st.markdown(f'<div class="warning-msg" style="padding: 0.75rem; margin: 0;">{html.escape(str(row["Prediction"]))}</div>', unsafe_allow_html=True)
                        else:
                            st.markdown(f'<div class="info-msg" style="padding: 0.75rem; margin: 0;">{html.escape(str(row["Prediction"]))}</div>', unsafe_allow_html=True)
                    
                    with col3:
                        st.write(f"📅 {html.escape(str(row['Date']))}")
        ...
        st.markdown(f'<div class="error-msg fade-in">Could not load analysis history: {html.escape(str(e))}</div>', unsafe_allow_html=True)
