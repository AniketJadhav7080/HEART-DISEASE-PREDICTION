...
class ECG:
	...
	def ModelLoad_predict(self,final_df):
		...
		"""
		try:
			# Try to load with compatibility mode for older scikit-learn versions
			loaded_model = joblib.load('Heart_Disease_Prediction_using_ECG.pkl')
		except Exception as e:
			# If there's a compatibility issue, try with different loading methods
			try:
				loaded_model = joblib.load('Heart_Disease_Prediction_using_ECG.pkl', mmap_mode='r')
				...
				if model_files:
					try:
						...
						loaded_model = joblib.load(model_files[0], mmap_mode='r')
				else:
					raise FileNotFoundError("Heart disease prediction model file not found")
