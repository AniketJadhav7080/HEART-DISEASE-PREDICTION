		...
		class RuleBasedClassifier:
			...
			def predict(self, X):
				...
				for i in range(len(X)):
					...
					# More balanced rule-based classification
					# Most ECGs should be normal, so we make normal the default
					if std_val > 0.4 and range_val > 0.8:  # Very high variability and range
						predictions.append(0)  # Abnormal Heartbeat
					elif mean_val > 0.8 and std_val > 0.3:  # Very high mean and high variability
						predictions.append(1)  # Myocardial Infarction
					elif mean_val > 0.7 and std_val < 0.15:  # High mean, very low variability
						predictions.append(3)  # History of MI
					elif std_val < 0.2 and mean_val < 0.5 and range_val < 0.4:  # Low variability, mean, and range
						predictions.append(2)  # Normal
					else:
						# Default to Normal for most cases (most ECGs are normal)
						predictions.append(2)  # Normal
