            ...
            if submit_login:
                if username and password:
                    ...
                    if success:
                        ...
                        st.markdown(f'<div class="error-msg fade-in">❌ {result}</div>', unsafe_allow_html=True)
                else:
        ...
        with st.form("register_form"):
            ...
            if submit_register:
                if not all([full_name, email, new_username, new_password, confirm_password]):
                    ...
                    if not is_valid:
                        st.markdown(f'<div class="error-msg fade-in">❌ {password_msg}</div>', unsafe_allow_html=True)
                    else:
                        success, result = register_user(new_username, email, new_password, full_name)
                        if success:
                            st.markdown(f'<div class="success-msg fade-in">✅ {result}</div>', unsafe_allow_html=True)
                        else:
                            st.markdown(f'<div class="error-msg fade-in">❌ {result}</div>', unsafe_allow_html=True)
...
def show_user_dashboard():
    ...
    if 'user' in st.session_state:
        ...
        <div class="user-dashboard fade-in">
            <h4 style="margin: 0 0 0.5rem 0; font-weight: 700;">👤 {user['full_name']}</h4>
            <p style="margin: 0; opacity: 0.9; font-size: 0.9rem;">@{user['username']}</p>
            <p style="margin: 0.5rem 0 0 0; opacity: 0.8; font-size: 0.8rem;">Role: {user['role'].title()}</p>
        </div>
        """, unsafe_allow_html=True)
